...
BEGIN
    ...
    ELSE
        q2 := q;
    END IF;
    FOR r IN SELECT id FROM obj_names WHERE position(q2 in name) > 0 order by position(q2 in name), name LIMIT lim
        LOOP
            FOR s IN SELECT o.pid as pid, o.id as id, o.name as name, o.desc as desc, o.fid as fid, f.name as fieldname
                     FROM objects o,
                          fields f
                     WHERE o.fid = f.id
                       and o.name_id = r.id
...
BEGIN
    ...
    IF EXISTS(SELECT * FROM pg_tables WHERE tablename = table_number) THEN
        ...
        IF id_column_name in (SELECT attname
                              FROM pg_attribute
                              WHERE attrelid =
                                    (SELECT oid FROM pg_class WHERE relname = quote_ident(table_number))) THEN
            FOR object_id in EXECUTE 'SELECT DISTINCT ' || quote_ident(id_column_name) || ' FROM ' ||
                                     quote_ident(table_number) || ' t WHERE ST_Intersects(t.the_geom, $1)' USING geom
                LOOP
                    RETURN QUERY EXECUTE
                                    'SELECT o.pid, o.id, o.name, o.desc as description, o.fid, o.bbox, o.area_km FROM objects o WHERE o.fid=' ||
                                    quote_literal(fidIn) || ' AND o.id=' || quote_literal(object_id);
                END LOOP;
        ELSE
            -- Otherwise just brute force search all of the objects
            RETURN QUERY EXECUTE
                        'SELECT o.pid, o.id, o.name, o.desc as description, o.fid, o.bbox, o.area_km FROM objects o WHERE o.fid=' ||
                        quote_literal(fidIn) || ' AND ST_Intersects(o.the_geom, $1)' USING geom;
        END IF;
    ELSE
        -- Otherwise just brute force search all of the objects
        RETURN QUERY EXECUTE
...
CREATE OR REPLACE FUNCTION search_objects_by_location(IN fidIn character varying, IN latitude double precision,
...
BEGIN
    ...
    IF EXISTS(SELECT * FROM pg_tables WHERE tablename = table_number) THEN
        ...
        IF id_column_name in (SELECT attname
                              ...
                              WHERE attrelid =
                                    (SELECT oid FROM pg_class WHERE relname = quote_ident(table_number))) THEN
            FOR object_id in EXECUTE 'SELECT ' || quote_ident(id_column_name) || ' FROM ' ||
                                     quote_ident(table_number) ||
                                     ' WHERE ST_Intersects(the_geom, ST_SetSRID(ST_Point(' || longitude || ', ' ||
                                     latitude || '),4326))'
                LOOP
                    RAISE NOTICE '%',object_id;
                    RETURN QUERY EXECUTE
                                    'SELECT * FROM objects o WHERE o.fid=' || quote_literal(fidIn) || ' AND o.id=' ||
                                    quote_literal(object_id);
                END LOOP;
        ELSE
            -- Otherwise just brute force search all of the objects
            RETURN QUERY EXECUTE 'SELECT * FROM objects o WHERE o.fid=' || quote_literal(fidIn) ||
                                 ' AND ST_Intersects(the_geom, ST_SetSRID(ST_Point(' || longitude || ', ' || latitude ||
                                 '),4326))';
        END IF;
    ELSE
        -- Otherwise just brute force search all of the objects
...
ALTER TABLE ONLY points_of_interest
    ALTER COLUMN id SET DEFAULT nextval('points_of_interest_id_seq'::regclass);
...
ALTER TABLE ONLY uploaded_objects_metadata
    ALTER COLUMN id SET DEFAULT nextval('uploaded_objects_metadata_id_seq'::regclass);
...
SELECT d.gid,
       ...
       d.family_lsid,
       d.genus_lsid,
       d.caab_species_number,
       d.caab_family_number,
       o.the_geom,
       o.name AS area_name,
       o.pid,
       d.type,
...
'
    ...
    FROM obj_names;
    UPDATE objects
    set namesearch=fields.namesearch,
        name_id=NULL
    FROM fields
    WHERE objects.fid = fields.id
      AND fields.namesearch <> objects.namesearch;
    INSERT
    INTO obj_names (name)
    SELECT lower(objects.name)
    FROM objects
             LEFT OUTER JOIN obj_names ON lower(objects.name) = obj_names.name
    WHERE obj_names.name IS NULL
      AND objects.namesearch = true
    GROUP BY lower(objects.name);
    UPDATE objects
    SET name_id=obj_names.id
    FROM obj_names
    WHERE lower(objects.name) = obj_names.name
      AND objects.namesearch = TRUE;
    SELECT count(*)
    FROM obj_names;'
