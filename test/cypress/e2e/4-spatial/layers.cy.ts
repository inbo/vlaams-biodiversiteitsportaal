describe("Spatial - Layers", () => {
  ...
  it("Can add contextual layer", () => {
    ...
    cy.get('ul.dropdown-menu[aria-labelledby="menu-0"')
      .contains("Laag")
      .click();
    cy.get('[testtag="availableLayers"]')
      .contains(layerName)
      .parent()
    ...
    cy.get(".progress-bar").should("not.be.visible");
    cy.get('[name="divMappedLayers"]').contains(layerName).should("be.visible");
  ...
  it("Can create a new layer from a filter on a contextual layer", () => {
    ...
    cy.get('[ng-show="wmsLegendVisible()"')
      .should("be.visible")
      .find("input[type='text'][placeholder='Filter']")
      .type(gemeente, { force: true });
    cy.get('[ng-show="wmsLegendVisible()"')
      .contains("Filter toepassen")
      .click({ force: true });
