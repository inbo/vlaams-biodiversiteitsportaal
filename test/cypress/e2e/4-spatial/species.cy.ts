...
describe("Spatial - Layers", () => {
    ...
    it("Can add a species group", () => {
        ...
        // Add a species group
        cy.get("#menu-0").click();
        cy.get('ul.dropdown-menu[aria-labelledby="menu-0"').contains("Soorten")
            .click();
    ...
    it("Can add species from species-list", () => {
        ...
        cy.get("[name='speciesList']").contains(speciesListName)
            .parent().parent().find(
                "input[type='checkbox']",
            ).check();
        cy.get('button[name="next"]').click();
    ...
    it.skip("Can add species by creating a new species-list", () => {
        ...
        cy.get("textarea[name='speciesInput']").type(
    ...
    it("Can add species from new imported points", () => {
        const datasetName = `Test species import points ${
...
"Cryphaea heteromalla","20250310T11:54:00+01:00",50.8791,4.7025,"TEST:1","test"`;
        ...
        cy.get('input[value="importPoints"]').parent().parent().contains(
            datasetName,
        );
        cy.get('button[name="next"]').click();
    ...
    it("Can add species from existing dataset", () => {
        ...
        cy.get('input[value="sandboxPoints"]')
            .parent().parent().contains(datasetName)
